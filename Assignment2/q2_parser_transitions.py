class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        if(transition == 'S'):
            ...
            self.dependencies.append((self.stack[-1],token))
        elif(transition == 'RA'):
            token = self.stack[-1]
            self.stack.pop()
            self.dependencies.append((self.stack[-1],token))
    ...
    while(len(unfinished_parses)):
        ...
        if(len(unfinished_parses)>=batch_size):
            ...
            while(len(parses)>0):
                ...
                parses = [parse for parse in parses if len(parse.stack) > 1 or len(parse.buffer) > 0]  
              
            del unfinished_parses[:batch_size]
            
        else:
            
            parses = unfinished_parses[:]
