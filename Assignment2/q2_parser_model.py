...
class ParserModel(Model):
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE
        embedding_tensor = tf.Variable(self.pretrained_embeddings,'embedding_tensor')
        embedding = tf.nn.embedding_lookup(embedding_tensor,self.input_placeholder)
        embeddings = tf.reshape(embedding,shape = [-1,self.config.n_features*self.config.embed_size])
        ### END YOUR CODE
        return embeddings
    ...
    def add_prediction_op(self):
        ...
        b1 = tf.get_variable('b1',dtype = tf.float32,shape = (self.config.hidden_size,),initializer = tf.constant_initializer())
        U = tf.get_variable('U',dtype = tf.float32,shape = (self.config.hidden_size,self.config.n_classes),
                            initializer = xavier_weight_init())
        b2 = tf.get_variable('b2',dtype = tf.float32,shape = (self.config.n_classes,),initializer = tf.constant_initializer())
