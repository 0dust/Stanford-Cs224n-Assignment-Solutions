...
def getSentenceFeatures(tokens, wordVectors, sentence):
    ...
    for word in sentence:
        ...
        sentVector += wordVectors[word_index,:]
...
def getRegularizationValues():
    ...
    ### YOUR CODE HERE
    values = np.logspace(-5, 7, num=100, base=10)
    #raise NotImplementedError
    ### END YOUR CODE
...
def outputPredictions(dataset, features, labels, clf, filename):
    ...
    with open(filename, "w") as f:
        ...
        for i in range(len(dataset)):
           ...
           print(f, "%d\t%d\t%s" % (labels[i], pred[i], " ".join(x.decode('utf-8') for x in dataset[i][0])), file =sys.stderr)
