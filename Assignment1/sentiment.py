...
def getSentenceFeatures(tokens, wordVectors, sentence):
    ...
    ### YOUR CODE HERE
    word_indices = list(map(lambda x:tokens[x],sentence))
    
    wordVectors_arr = wordVectors[word_indices]
    sentVector = np.mean(wordVectors_arr,axis = 0).flatten()
...
def getRegularizationValues():
    ...
    ### YOUR CODE HERE
    values = [1.1,0.0001,0.01,0.002,3,2.3]
#     raise NotImplementedError
    ### END YOUR CODE
...
def chooseBestModel(results):
    ...
    testx = results[0]['test']
    for i, result in enumerate(results):
        
        if(testx < result['test']):
            testx= result['test']
            index = i
...
def outputPredictions(dataset, features, labels, clf, filename):
    ...
    with open(filename, "w") as f:
        print(f, "True\tPredicted\tText",file=sys.stderr)
        for i in xrange(len(dataset)):
            print(f, "%d\t%d\t%s" % (
                labels[i], pred[i], " ".join(dataset[i][0])),file=sys.stderr)
