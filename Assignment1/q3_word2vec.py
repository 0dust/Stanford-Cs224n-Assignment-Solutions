...
def normalizeRows(x):
    ...
    ### YOUR CODE HERE
    squared = np.square(x)
    squared_sum = np.sum(squared,axis = 1,keepdims = True)
    root = np.sqrt(squared_sum)
    x = (x)/(root)
...
def softmaxCostAndGradient(predicted, target, outputVectors, dataset):
    ...
    ### YOUR CODE HERE
    dot_pro = np.dot(outputVectors, predicted)
    
    yhat = softmax(dot_pro)
...
def skipgram(currentWord, C, contextWords, tokens, inputVectors, outputVectors,
    ...
    for i in contextWords:
        ...
        c_cost, c_grad_in, c_grad_out = \
            word2vecCostAndGradient(predicted, u_idx, outputVectors, dataset)
        cost += c_cost
