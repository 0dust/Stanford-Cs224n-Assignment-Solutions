...
class GRUCell(tf.nn.rnn_cell.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope):
            ...
            U_r = tf.get_variable('U_r',initializer = tf.contrib.layers.xavier_initializer(),shape = (self.state_size,self.state_size))
            W_r = tf.get_variable('W_r',initializer = tf.contrib.layers.xavier_initializer(),shape = (self.input_size,self.state_size))
            b_r = tf.get_variable('b_r',initializer = tf.constant_initializer(),shape = (self.state_size))
            
            U_z = tf.get_variable('U_z',initializer = tf.contrib.layers.xavier_initializer(),shape = (self.state_size,self.state_size))
            W_z = tf.get_variable('W_z',initializer = tf.contrib.layers.xavier_initializer(),shape = (self.input_size,self.state_size))
            b_z = tf.get_variable('b_z',initializer = tf.constant_initializer(),shape = (self.state_size,))
            
            U_o = tf.get_variable('U_o',initializer = tf.contrib.layers.xavier_initializer(),shape = (self.state_size,self.state_size))
            W_o = tf.get_variable('W_o',initializer = tf.contrib.layers.xavier_initializer(),shape = (self.input_size,self.state_size))
            b_o = tf.get_variable('b_o',initializer = tf.constant_initializer(),shape = (self.state_size,))
