...
class RNNCell(tf.nn.rnn_cell.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope):
            ...
            b  =  tf.get_variable('b',initializer  = tf.constant_initializer(),shape = (self.state_size,))
            
            new_state = tf.sigmoid(tf.matmul(inputs,W_x) + tf.matmul(state,W_h) + b)
           # U = tf.get_variable('U',shape = (self.state_size,self.output_size),initializer = tf.contrib.layers.xavier_initializer())
