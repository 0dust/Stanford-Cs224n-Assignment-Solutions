...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    for sentence, labels in data:
        ...
        for i in range(len(sentence)):                                            #([5,0] + [1,1] + [2,0], 1,),
            ...
            beg_list = []
            end_list = []
            s_index = i-window_size
            if(s_index<0):
                s_index = s_index*(-1)
                beg_list = start*s_index
                s_index = 0
            e_index = i+window_size+1
            if(e_index>len(sentence)):
                e_index = e_index-len(sentence)
                end_list = end*e_index
                e_index = len(sentence)
            sent  = []
            for j in range(s_index,e_index):
                sent.extend(sentence[j])
            beg_list.extend(sent)
            if(len(end_list) != 0):
...
#        else:
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE (!3-5 lines)
        embedding_tensor =tf.Variable(self.pretrained_embeddings,name = 'embedding_tensor')
    ...
    def add_prediction_op(self):
        ...
        W = tf.get_variable('weightW',initializer = tf.contrib.layers.xavier_initializer(),shape = (self.config.n_window_features*self.config.embed_size,self.config.hidden_size))
        b1 = tf.get_variable('variableb1',initializer = tf.contrib.layers.xavier_initializer(),shape = (1,self.config.hidden_size))
        
        h = tf.matmul(x,W)
        h = tf.add(h,b1)
        h = tf.nn.relu(h)
        h_drop = tf.nn.dropout(h,keep_prob = dropout_rate)
        U = tf.get_variable('weightsU',initializer = tf.contrib.layers.xavier_initializer(),shape = (self.config.hidden_size,self.config.n_classes))
        b2 = tf.get_variable('variableb2',initializer = tf.contrib.layers.xavier_initializer(),shape = (1,self.config.n_classes))
